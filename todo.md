...
## Active priorities (updated 2026-05-19, USMCA section closed)
...
2. **Finish the biggest post-annex modeling gaps**: Russia clause (8) full smelter/cast origin logic, UK 95% qualifying-content blending, Annex IV exception buckets, and product-condition exemptions like 9903.81.92 are still approximated or unmodeled. (9903.82.01 zero-metal-content is now scaffolded but dormant — calibration is its own line item below.)
3. **Then clear secondary rebuild/calibration debt**: calibrate semi/annex/zmc assumptions, and tackle the remaining low-priority performance and cleanup items. (The former "rerun the OOM-failed post-build alternatives" clause is dropped — see item 4.)
4. **Rip out the `--with-alternatives` machinery (ANCIENT — flagged 2026-06-06).** The 6 USMCA rebuild-alternatives re-run the full revision loop 3–4× (a 10–12h job) and bloat the internal publish; they're obsolete. Remove the flag + all its code paths: `--with-alternatives` / `--alternatives-only` in `src/00_build_timeseries.R`, the `--with-alternatives` line + walltime rationale in `scripts/submit_publish_internal.sh`, and the scenario-publish handling in `src/publish_internal.R` (`include_scenarios`, the `scenarios/<name>` block). Also retire the stale `output/scenarios/` outputs they leave behind (the no_ieepa scenario got auto-swept into the 2026-06-06 publish from a 06-03 leftover — see `docs/master_fix_integration.md`). Default publishes to `actual/`-only.
