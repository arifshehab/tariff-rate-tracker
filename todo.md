...
1. **TPC validation of the rebuilt series (rebuild itself DONE 2026-06-04).** Full rebuild completed on master with all six fixes: 43 snapshots, rate_timeseries.rds + parquet (208.9M rows, streaming combine), daily series, quality report, 7/7 post-build scenarios (incl. eu_auto_25pct and the previously-OOM no_* set), weighted ETR + TPC plots. The TPC policy-aligned comparison now shows tracker BELOW TPC: Apr 17 −9.71pp, Oct 17 −4.23pp, Nov 17 −2.6pp, Jul −1.78pp. Partly expected (the eta fixes deliberately move statutory toward Census-observed application, e.g. 8471 auto-parts exclusion, USMCA share fixes — TPC models margins the tracker now de facto excludes), but the April gap is dominated by the rev_12 dating problem (next item). No pre-fix reference table survives locally (outputs overwritten) — re-run `compare_etrs.R` + `test_tpc_comparison.R` and decompose the deltas before reading too much into aggregates. Then hand new snapshots to tariff-etr-eval for a fresh eta panel.
2. **Re-date revision_dates.csv against the official change records (NEW, discovered 2026-06-04).** The CSV's effective dates for rev_25–31 do not match the USITC change records in `data/hts_change_record/`: rev_25 is dated 2025-09-26 but was published Oct 14 with items effective Oct 14 (PP 10976 wood); rev_26 dated 10-06 vs items effective Nov 1 (MHD 232); rev_29 dated 10-31 vs published Nov 17 with items effective Nov 13 (ag Annex II EO). **Also rev_12: dated 2025-04-14 but its content is the China 125%→34% Geneva suspension — Geneva was May 12, 2025.** This alone makes the tracker drop China to baseline a month early and plausibly explains most of the −9.7pp Apr 17 TPC gap (TPC has China at 125%+20% there). The daily series is therefore time-shifted in BOTH the Apr–May and Sept–Dec 2025 windows. Fix: systematic pass deriving each revision's policy effective date from its change record (all records are local). The Annex-II date stamping already uses change-record legal dates, so the exempt windows are immune; the rest of each revision's content is not.
