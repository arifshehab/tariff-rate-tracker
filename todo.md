...
## Active priorities (updated 2026-06-04, extreme-eta review fixes added at top)

1. **Extreme-eta review fixes** (see section below; from `docs/tracker_review_extreme_etas.md` + verification 2026-06-04). Six items in dependency order: (1) 8-digit leaf product drop, (2) 8471 auto-parts sweep, (3) India EO + ch98 per-EO exempt entries, (4) re-run `expand_ieepa_exempt.R` post-audit, (5) effective-date the IEEPA exempt list, (6) USMCA eligibility false negatives.
...
- [ ] **1. Keep 8-digit leaf HTS lines (universe gap, 473 lines).** `is_valid_hts10()` requires exactly 10 digits, so `04_parse_products.R:70` silently drops HTS lines that are leaves at 8 digits (no statistical suffix): 378 ch98 + 95 ch91 lines in 2026_rev_2, including the entire Swiss watch complex (9101.21.50, 9102.21.70, …). Census reports these as HTS10 + "00". Fix: retain 8-digit leaves (those with no 10-digit children), pad to 10 with "00". Do FIRST — it changes the product universe items 3–5 key off. NOT a compound-rate parser drop (compound rates are kept, base coalesced to 0).
- [ ] **2. Un-sweep general-purpose computers from 232 auto parts ($17.6B Taiwan ch84 gap).** `s232_auto_parts.txt` contains bare prefix `8471`, so 23 of 28 Taiwan 8471 lines get rate_232 = 25% − auto rebate = 23.7625% as AUTO PARTS. Census: ~0.8% collected on $139B. The Apr-28 semi binary calibration only covers the 5 codes on `s232_semi_products.csv`. Fix: scope the 8471 auto-parts entry to what Note 33(g) actually enumerates / what CBP de facto collects; companion: check 8473.21/.29/.30/.50 against Annex II (9 of 14 Taiwan 8473 lines pay 20% reciprocal).
- [ ] **3. Populate per-EO exempt lists: India Annex II inheritance + ch98 across all EOs (~$2.3B).** The country-EO Annex II bypass (6ebcd6b, `docs/country_eo_annex_overshoot.md`) is correct for Brazil but `country_eo_exempt_products.csv` has 0 entries under 3004 and only 7 ch98 entries. India pharma charged 25%, Brazil/India 9801 charged 40%/25% in current snapshots; Census ≈ 0. Fix: India EO (9903.01.84) inherits EO 14257 Annex II (verify legal text); ch98 exempt across all IEEPA layers per Note 2(v)(i). Do NOT re-route country EOs through Annex II globally — that resurrects the Brazil coffee overshoot.
- [ ] **4. Re-run `expand_ieepa_exempt.R` (ch97 Berman never materialized, $0.3B+).** Fixes 4–5 (ch97/ch49) were added to the script 2026-03-23 but `ieepa_exempt_products.csv` was last regenerated 2026-03-10 — the script was never re-run. 0 ch97 codes on the list; France ch97 charged 15%. Re-run on top of the audited list (2026-04-28 audit removed 26 over-broad entries — confirm Fix 4/5 don't revert those) and after item 1 (new universe).
- [ ] **5. Effective-date the IEEPA exempt list (negative-eta ag cluster).** `ieepa_exempt_products.csv` is static, so the Nov 14, 2025 agricultural Annex-II expansion (coffee 0901, tea 0902, flowers 0603, cocoa 1801, palm 1511, …) applies retroactively — rev_25 (Oct 2025) snapshot shows reciprocal = 0 on Colombia coffee/flowers. Fix: add `effective_date_start` column + loader filter, copying the schema already working in `country_eo_exempt_products.csv` (lighter than per-revision lists).
- [ ] **6. USMCA eligibility false negatives (CA gold $0.9B + CA crude, part of $1.6B ch27).** Canada 7108 carries full 40% fentanyl with `usmca_eligible = FALSE` on 9/10 lines (Census: 0.19% collected); 2709.00.20.10 FALSE → full 10% energy-carveout rate while sibling lines are ≈0. Likely shared root cause in S/S+ `special`-field extraction (MFN-free lines have empty special fields) and/or missing per-HTS10 DataWeb shares defaulting to 0 instead of falling back to HS8. NOT a `fentanyl_carveout_products.csv` gap — that list carves out at 10%, not the ≈0 Census shows. Also evaluate the Sept 2025 bullion clarification as a direct 7108 exemption channel.
