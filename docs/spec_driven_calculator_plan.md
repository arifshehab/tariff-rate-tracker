...
- **2026-06-06 — Pass-2 / P2-1 (unified timeline splitter) — DONE + GOLDEN RE-FROZEN.** Build (array `13905511` → gather `13905512`, policy grid) minted the 3 `bnd_` snapshots; impact vs prior golden `70b6b97` is surgical — **6 daily rows move, all inside the boundary windows, 0 outside** (regression guard green): 2025-03-12/13 +0.22pp unweighted / +0.54pp `etr_232` (§232 metal-exemption expiry), 2026-02-20→23 IEEPA reciprocal+fentanyl → 0 (−9.2pp unweighted / weighted_etr 13.1%→7.5%, the 4-day pre-S122 window). `test_timeline_invariants` **16/16 green** vs the real build. **New golden captured: `tests/golden/6ec81b9`** (45 snapshots = 42 real + 3 `bnd_`; weighted daily; `policy_params_md5 361cf48e…`; `70b6b97` retained for provenance). Commits `1f07a3e` (mechanism) + `6ec81b9` (validation + honest §301 invariant + weighted-chart/impact tooling). **Discovered modeling gap (flagged, NOT fixed):** the 2026-11-10 §301 cranes/chassis codes `9903.91.12-.16` carry a parsed 100% rate in the Ch99 text but are absent from `section_301_rates`, so that mint is currently daily-inert (forward-compatible — activates once priced). Note: `docs/timeline_split_integration.md`.
